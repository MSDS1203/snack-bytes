...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        const docRef1 = doc(db, "solitaireLeaderboard", uid);
        console.log("getting the doc reference for solitaire leaderboard: ", !!docRef1);
        ...
        if (docSnap.exists()) {
            console.log("Current score for", uid, ":", docSnap.data()["solitaire"]);
            currHighScore = docSnap.data()["solitaire"];
        ...
        submitScore.addEventListener("click", async function() {
            ...
            newHighScore = Number(localStorage.getItem("currentScoreSolitaire"));
            console.log("New score to be added: ", newHighScore);
        
            try{
                if (docSnap.exists()) {
                    console.log("Document data:", docSnap.data()["solitaire"]);
                    ...
                    if(newHighScore < currHighScore || currHighScore === 0)
                    {
                        ...
                        await setDoc(docRef, { solitaire: newHighScore }, { merge: true }); 
                        await setDoc(docRef1, { solitaire: newHighScore }, { merge: true });
